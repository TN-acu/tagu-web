...
async function setupQuiz() {
    try {
        ...
        const response = await fetch('quiz_data_iryoutoshakai2025zenki_kimatu.txt');
        ...
        for (let i = 0; i < lines.length; i += 5) {
            ...
            if (question === '' || correctAnswer === '' || finalChoices.length < 2) { // 選択肢が2つ以上あればOK
...
function selectChoice(quizIndex, choiceIndex) {
    ...
    const selectedBtn = visibleChoiceButtons[choiceIndex];

    visibleChoiceButtons.forEach(btn => btn.classList.remove('selected'));
    
    // 正しいボタンを見つけるためにtextContentで比較
    let clickedButton;
